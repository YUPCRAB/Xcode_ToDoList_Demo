...
class ToDoTableViewController: UITableViewController {
    ...
    private var taskList = Task.getInitData()
    ...
    func addNewTask()
    {
        self.displayCustonAlert(isNewTask: true, indexPath: nil, title: "New Task", message: "Enter details for new task")
    }
    
    private func displayCustonAlert(isNewTask : Bool, indexPath : IndexPath?, title : String, message : String)
    {
        ...
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: {_ in
            if let title = alert.textFields?[0].text, let subtitle = alert.textFields?[1].text
            {
                if (isNewTask)
                {
                    self.addTaskToList(title : title, subtitle : subtitle)
                }else if (indexPath != nil)
                {
                    self.updateTaskList(indexPath: indexPath!, title: title, subtitle: subtitle)
                }
            }
        }))
    ...
    private func updateTaskList(indexPath : IndexPath, title : String, subtitle : String)
    {
    ...
    @objc
    private func handleLongPress(_ gestureRecogniser : UILongPressGestureRecognizer)
    {
        if gestureRecogniser.state == .ended
        {
            let touchPoint = gestureRecogniser.location(in: self.tableView)
            
            if let indexPath = self.tableView.indexPathForRow(at: touchPoint)
            {
                self.displayCustonAlert(isNewTask: false, indexPath: indexPath, title: "Edit Task", message: "Please provide the updated details")
            }
        }
    }
